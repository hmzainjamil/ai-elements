...
> **AI elements skill library — reusable UI components generated by Claude**
...
| Concept | Description |
|---|---|
| **Elements** | Core primitive of ai elements — fundamental building block |
| **Ui** | Execution layer handling UI operations and state |
| **Components** | Integration point connecting components to external services |
| **Claude** | Configuration schema for claude behavior and routing |
| **Generated** | Optimization layer reducing generated overhead and latency |
...
/skills load ai-elements

# Check skill status
/skills list | grep ai

# Clone and explore locally
gh repo clone hmzainjamil/ai-elements
cd ai-elements

# Run with Claude Code (invoke skill directly)
claude -p "use ai elements skill to handle my task"
...
- **Digital agencies** — automate elements workflows, cut delivery time 10×
- **SaaS founders** — plug into existing Claude Code setup, zero infra overhead
- **Freelancers** — use as force-multiplier for client work in UI + components
...
cp -r . ~/.claude/skills/ai-elements/

# Option 3: Via gh CLI
gh repo clone hmzainjamil/ai-elements ~/.claude/skills/ai-elements
```
...
response = client.messages.create(
    ...
    max_tokens=4096,
    system="You are using the ai-elements skill. AI elements skill library — reusable UI components generated by Claude",
...
mae run "ai elements - execute primary workflow"

# Blast across multiple models
tcc blast "task1 for elements" "task2 for UI" "task3 for components"
```
...
```yaml
# .claude/skills/ai-elements/config.yaml
skill:
  name: ai-elements
  version: "1.0.0"
  triggers:
...
User Intent
    │
    ▼
Skill Router ──► Trigger Match ──► ai-elements
                                       │
                    ┌──────────────────┼──────────────────┐
                    ▼                  ▼                   ▼
  ...
  "nodes": [
    ...
    },
    {
      ...
      "parameters": {
        ...
        "body": {{"skill": "ai-elements", "input": "{{$json.body}}"}}
      }
    }
  ]
}
...
def run_skill(task: str) -> str:
    result = subprocess.run(
        ['claude', '-p', f'using ai-elements skill: {task}'],
        capture_output=True, text=True
    )
    return result.stdout

output = run_skill("elements analysis for my project")
print(output)
```
...
1. Fork this repo
2. Create feature branch: `git checkout -b feature/elements-improvement`
3. Add/update `SKILL.md` with new capabilities
4. Test: `claude -p "test ai-elements skill"`
...
### v1.0.0
- Initial release with core elements functionality
